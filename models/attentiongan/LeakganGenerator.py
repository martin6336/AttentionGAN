...
class Generator(object):
    def __init__(self, sequence_length, num_classes, num_vocabulary,
                 ...
                 goal_size, step_size, sess, D_model, end_token, att_model=None, LSTMlayer_num=1, l2_reg_lambda=0.0, learning_rate=0.01,
                 drop_out=1):
        ...
        self.num_filters_total = emb_dim
        ...
        self.att_feature_extractor = self.att_model.extract_feature
        ...
        #todo padding 需要改成end_token
        self.padding_array = tf.constant(self.end_token, shape=[self.batch_size, self.sequence_length], dtype=tf.int32)
        ...
        gen_o_worker_array = tensor_array_ops.TensorArray(dtype=tf.float32, size=int(self.sequence_length / self.step_size),
                                                          dynamic_size=False, infer_shape=True, clear_after_read=False)
        # print('recurrence')
        def _g_recurrence(i, begin, x_t, h_tm1, h_tm1_manager, gen_o, gen_x, goal, last_goal, real_goal, step_size,
                          gen_real_goal_array, gen_o_worker_array):
            # padding sentence by -1
            # cur_sen当前被截取的句子
            # gen_x的size是i*batch_size
            # 这个concat拼接后是(i+sequence_length, batch),所以split后有两个，分别是sequence_length和i，取第一个
            ...
            # print(self.padding_array.shape)
            # print(cur_sen.shape)
            with tf.variable_scope(self.scope):
                # feature = self.FeatureExtractor_unit(cur_sen, self.drop_out)
                # print('cur_sen',cur_sen.shape)
                feature = self.att_model.extract_feature(cur_sen, begin)
            ## !!!!!!!!!!!!1addddddd
            # feature 维度应该为self.num_filters_total
            # print('feature')
            # print(feature.shape)
            h_t_Worker = self.g_worker_recurrent_unit(x_t, h_tm1)  # hidden_memory_tuple
            # h_t_Worker是隐含层，这个函数有w会把其维度转换为self.num_vocabulary*self.goal_size
            ...
            return i + 1, begin+1, x_tp1, h_t_Worker, h_t_manager, gen_o, gen_x, goal, \
        ...
        # satart_token是0，但是oracle文件中没有0
        # begin=tf.constant(0, dtype=tf.int32)
        ##!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!这里sequence_length没有+1
        begin=0
        _, _, _, _, _, self.gen_o, self.gen_x, _, _, _, _, self.gen_real_goal_array, self.gen_o_worker_array = control_flow_ops.while_loop(
            cond=lambda i, _1, _2, _3, _4, _5, _6, _7, _8, _9, _10, _11, _12: i < self.sequence_length,
            body=_g_recurrence,
            loop_vars=(tf.constant(0, dtype=tf.int32), begin,
        ...
        # print('pretrain')
        def preTrain(i, begin, x_t, g_predictions, h_tm1, input_x, h_tm1_manager, last_goal, real_goal, feature_array,
            ...
            with tf.variable_scope(self.scope):
                # feature = self.FeatureExtractor_unit(cur_sen, self.drop_out)
                feature = self.att_model.extract_feature(cur_sen,begin)
                # print('pretrain extract feature dim',feature.shape)
            ...
            return i + 1, begin+1, x_tp1, g_predictions, h_t_Worker, input_x, h_t_manager, \
        ...
        begin=0
        _, _, _, self.g_predictions, _, _, _, _, _, self.feature_array, self.real_goal_array, self.sub_feature, self.all_sub_features, self.all_sub_goals = control_flow_ops.while_loop(
            cond=lambda i, _1, _2, _3, _4, _5, _6, _7, _8, _9, _10, _11, _12, _13: i < self.sequence_length + 1,
            body=preTrain,
            loop_vars=(
            tf.constant(0, dtype=tf.int32), begin, tf.nn.embedding_lookup(self.g_embeddings, self.start_token),  g_predictions,
            self.h0_worker,
        ...
        pretrain_goal_summary=tf.summary.scalar('pretrain_goal_loss', self.pretrain_goal_loss)
        ...
        pretrain_worker_summary=tf.summary.scalar('pretrain_worker_loss', self.pretrain_worker_loss)
        ...
        goal_summary=tf.summary.scalar('goal_loss', self.goal_loss)
        ...
        worker_summary=tf.summary.scalar('worker_loss', self.worker_loss)
        ...
        self.merge_summary=tf.summary.merge([pretrain_goal_summary,pretrain_worker_summary])
        self.ad_summary = tf.summary.merge([goal_summary,worker_summary])
    ...
    def rollout(self, input_x, given_num):
        ...
        def _g_recurrence_1(i, begin, x_t, input_x, gen_x, h_tm1, h_tm1_manager, last_goal, real_goal, give_num):
            cur_sen = \
            tf.split(tf.concat([tf.split(input_x, [i, self.sequence_length - i], 1)[0], self.padding_array], 1),
            ...
            return i + 1, begin+1, x_tp1, input_x, gen_x, h_t_Worker, h_t_manager, \
        ...
        def _g_recurrence_2(i, begin, x_t, gen_x, h_tm1, h_tm1_manager, last_goal, real_goal):
            # with tf.device('/cpu:0'):
            cur_sen = tf.cond(i > 0, lambda:
            tf.split(tf.concat([tf.transpose(gen_x.stack(), perm=[1, 0]), self.padding_array], 1),
            ...
            return i + 1, begin+1, x_tp1, gen_x, h_t_Worker, h_t_manager, \
                   tf.cond(((i) % self.step_size) > 0, lambda: real_sub_goal,
                           lambda: tf.constant(0.0, shape=[self.batch_size, self.goal_out_size])), \
        ...
        i, begin, x_t, _, gen_for_reward, h_worker, h_manager, self.last_goal_for_reward, self.real_goal_for_reward, given_num = control_flow_ops.while_loop(
            cond=lambda i, _1, _2, _3, _4, _5, _6, _7, _8, given_num: i < given_num + 1,
            body=_g_recurrence_1,
            loop_vars=(
            tf.constant(0, dtype=tf.int32), begin, tf.nn.embedding_lookup(self.g_embeddings, self.start_token), self.x,
            gen_for_reward,
        ...
        _, _, _, gen_for_reward, _, _, _, _ = control_flow_ops.while_loop(
            cond=lambda i, _1, _2, _3, _4, _5, _6, _7: i < self.sequence_length + 1,
            body=_g_recurrence_2,
            loop_vars=(
            i, begin, x_t, gen_for_reward, h_worker, h_manager, self.last_goal_for_reward, self.real_goal_for_reward),
    ...
    def pretrain_step(self, sess, x, dropout_keep_prob):
        outputs = sess.run([self.pretrain_worker_updates, self.pretrain_worker_loss, self.pretrain_manager_updates,
                            self.pretrain_goal_loss,self.merge_summary],
    ...
    def create_Worker_recurrent_unit(self, params):
        with tf.variable_scope('Worker'):
            ...
            def unit(x, hidden_memory_tm1):
                ...
                # Input Gate
                #！！！！！！！！！ 报错会tf.matmul(previous_hidden_state, self.Ui_worker)，但是这里并没错，上面那行错了
                i = tf.sigmoid(
    ...
    def create_Manager_recurrent_unit(self, params):
        with tf.variable_scope('Manager'):
            ...
            def unit(x, hidden_memory_tm1):
                ...
                # Input Gate
                # print(self.Wi.shape)
                # print(previous_hidden_state.shape)
                # print(self.Ui.shape)
                i = tf.sigmoid(
                    tf.matmul(x, self.Wi) +
