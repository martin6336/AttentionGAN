        ...
        with tf.variable_scope('discriminator'):
            ...
            for filter_size, num_filter in zip(filter_sizes, num_filters):
                with tf.name_scope("conv-maxpool-%s" % filter_size):
                    ...
                    # self.embedded_chars_expanded [None, sequence_len, emd_dim]
                    # [filter_size, emd_dim, 1, out_channels]
                    # out: [None, sequence_len-filter_size+1, 1, out_channels]
                    conv = tf.nn.conv2d(
            ...
            # 在哪一个维度concat，那么这个维度的数量变多
            self.h_pool = tf.concat(pooled_outputs, 3)
            # 上面就是把信息conv后，然后全部拼接到一个向量
