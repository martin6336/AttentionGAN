...
class Generator(object):
    def __init__(self, sequence_length, num_classes, num_vocabulary,
        ...
        with tf.variable_scope('Manager'):
            ...
            self.goal_init = tf.get_variable("goal_init",
                                             initializer=tf.truncated_normal([self.batch_size, self.goal_out_size],
                                                                             stddev=0.1))
            # manager_params中包含lstm各个参数以及开始的这个goal向量
        ...
        def _g_recurrence(i, x_t, h_tm1, h_tm1_manager, gen_o, gen_x, goal, last_goal, real_goal, step_size,
            ...
            #############################################################worker
            # h_t_Worker是隐含层，第一个x_t是start_token对应的向量
            h_t_Worker = self.g_worker_recurrent_unit(x_t, h_tm1)  # hidden_memory_tuple
            # 这个函数有w会把其维度转换为self.num_vocabulary*self.goal_size
            ...
            #############################################################manager
            # h0_manager是起始h_tml_manger
            # feature每次都是现有句子的信息提取
            ...
            # real_goal每隔step_size变化一次，step_size内goal是一样的
        ...
        def preTrain(i, x_t, g_predictions, h_tm1, input_x, h_tm1_manager, last_goal, real_goal, feature_array,
            ...
            # size=int(self.sequence_length / self.step_size)
            # 每个位置上都是相应step_size的起点，记录的是extract的feature相较于上个step_size的变化
        ...
        ######################################### 这里的loss是goal向量和sub_feature向量的相似度衡量
        ...
        with tf.name_scope("Manager_PreTrain_update"):
            ...
            # 这里的pretrian_loss只关于manager_param求导，只更新和manager相关的系数
        ...
        #######################################################就是worker对实际各个单词的预测概率越大越好，根据o和w相乘
        ...
        ################################################################和reward相关
        # reward只和goal_loss相关，所以也只和manager的更新相关
